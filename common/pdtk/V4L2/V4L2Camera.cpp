...
//    }
//
    pRawImage_ = std::unique_ptr<unsigned char>(new unsigned char[buf.length]);
    start = true;
}
...
int V4L2Camera::GrabRawFrame(void *raw_base)
{
    ...
    LOGDO("copy size :%d", buf.bytesused);
...
int V4L2Camera::getMonoData(unsigned char *const gray) {
    int ret = GrabRawFrame(pRawImage_.get());
    if(ret < 0){
        return ret;
    }
    return yuyv2gray(pRawImage_.get(), gray);
}

int V4L2Camera::yuyv2gray(const unsigned char* yuyv, unsigned char* const gray) {
    static const int LEN = width * height;
    for(int i = 0; i < LEN; ++i){
        gray[i] = yuyv[i << 1];
    }
    return LEN;
...
int V4L2Camera::setPreviewSize(int width, int height, int pixformat) {
    ...
    LOGD("setPreviewSize %d, %d, %d", width, height, pixformat);
...
void V4L2Camera::_start() {
    ...
    LOGE("_start raw buf.length %d", buf.length);
...
void V4L2Camera::renderVideo(const unsigned char *preview) {
    std::lock_guard<std::mutex> lock(windowLock);
    if (window == 0) {
        return;
    }
    LOGEO("RenderVideoElement width:%d, height:%d", width,height);
...
void V4L2Camera::sendDataToJava(unsigned char *raw) {
    ...
    LOGD("pixFormat %d, size : %d  ", pixelformat, size);
