...
public:
    ...
    void _start();
    void renderVideo(const unsigned char *preview);

    void setListener(JavaCallHelper * listener);
    void sendDataToJava(unsigned char *raw);

    int getMonoData(unsigned char* const gray);

    int yuyv2gray(const unsigned char* yuyv, unsigned char* const gray);
...
private:
	...
	std::unique_ptr<unsigned char> pRawImage_;
};
