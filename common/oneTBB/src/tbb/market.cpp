...
market::market(unsigned workers_soft_limit, unsigned workers_hard_limit, size_t stack_size)
    : my_num_workers_hard_limit(workers_hard_limit), my_num_workers_soft_limit(workers_soft_limit)
#if __TBB_TASK_PRIORITY
      ,
    ...
    if (m) {
        ...
        if (workers_requested != governor::default_num_threads() - 1) {
            ...
            if (soft_limit_to_report < workers_requested) {
                runtime_warning(
                    "The number of workers is currently limited to %u. "
                ...
                internal::as_atomic(m->my_workers_soft_limit_to_report).compare_and_swap(skip_soft_limit_warning, soft_limit_to_report);
            }
        }
        if (m->my_stack_size < stack_size)
            runtime_warning(
                "Thread stack size has been already set to %u. "
...
bool market::update_arena_priority(arena& a, intptr_t new_priority) {
    ...
    if (p == my_global_bottom_priority) {
        ...
        __TBB_ASSERT(p < new_priority, NULL);
        __TBB_ASSERT(new_priority <= my_global_top_priority, NULL);
        while (my_global_bottom_priority < my_global_top_priority && !my_priority_levels[my_global_bottom_priority].workers_requested)
